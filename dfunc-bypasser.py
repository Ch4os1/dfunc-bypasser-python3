...
if args.url:
    ...
    with open(phpinfofile, 'r') as f:
        phpinfo = f.read()
else:
    parser.print_help()   # removed erroneous 'print' that printed None
    exit()

modules = []
inp = []

inp = phpinfo.split('disable_functions<td class="v">')[1].split("</")[0].split(',')[:-1]
...
if "PHP-FPM":   # ← this condition is always true (string is non‑empty)
